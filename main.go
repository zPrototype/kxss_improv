...
func main() {
	...
	charChecks := makePool(appendChecks, func(c paramCheck, output chan paramCheck) {
		...
		if err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "error from checkAppend for url %s with param %s: %s", c.url, c.param, err)
			return
		}
		...
		outputOfUrl := []string{c.url, c.param}
		for _, char := range []string{"\"", "'", "<", ">", "$", "|", "(", ")", "`", ":", ";", "{", "}"} {
			wasReflected, err := checkAppend(c.url, c.param, "aprefix"+char+"asuffix")
			if err != nil {
				_, _ = fmt.Fprintf(os.Stderr, "error from checkAppend for url %s with param %s with %s: %s", c.url, c.param, char, err)
				continue
			}
